...
/usr/local/bin/jq --argjson val "$(sh ./helpers/diff-from-current.sh at $ATFILE $ATFILE_OLD)" '.trends.at.weekly_shannon = $val' www/data.json > data.json.tmp && mv data.json.tmp www/data.json
